...
const copyImageFilesAll = function (fileImagePath_1, writeIamagePath_1) {
  for (var _len = arguments.length, args_1 = new Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
    args_1[_key - 2] = arguments[_key];
  }
  return __awaiter(undefined, [fileImagePath_1, writeIamagePath_1, ...args_1], undefined, function (fileImagePath, writeIamagePath) {
...
const findFileAndWrite = (filename, to, options) => __awaiter(undefined, undefined, undefined, function* () {
  const check = yield checkFileExists(to);
  mdContent = yield getFileAll(options, path$1.join(filename, "/**").replace(/\\/g, "/"));
  if (!mdContent.length) {
    return console.log("no file");
  }
  yield handleFile(check, to);
  yield copyImageFilesAll(filename, to);
  const result = yield createFile(mdContent);
  const readFileContent = yield readFile(result);
  const file = yield FileDir.getFile(to);
  yield writeFileAll(readFileContent, path$1.join(to, file.name + options.ext));
});
// 获取当前目录下的所有文件和文件夹
const readCatalogue = function (findPosition_1, writingPosition_1) {
  for (var _len2 = arguments.length, args_1 = new Array(_len2 > 2 ? _len2 - 2 : 0), _key2 = 2; _key2 < _len2; _key2++) {
    args_1[_key2 - 2] = arguments[_key2];
  }
  return __awaiter(undefined, [findPosition_1, writingPosition_1, ...args_1], undefined, function (findPosition, writingPosition) {
    ...
    return function* () {
      ...
      writingPosition = Array.isArray(writingPosition) ? writingPosition : [writingPosition];
      const defaultOption = {
        regexp: options.exclude || null,
      ...
      for (let i = 0; i < findPosition.length; i++) {
        yield findFileAndWrite(findPosition[i], writingPosition[i], defaultOption);
      }
    }();
  });
};
...
//# sourceMappingURL=chunk-index-C_XoNQG2.esm.js.map
